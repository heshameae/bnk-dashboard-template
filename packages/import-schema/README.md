...
9. The report opens with the handover path and its SHA-256, then: files written, files in `sources/` not in this handover (left untouched), tables skipped, questions for the data team, rows set aside, and the contract's standing ask. Re-running on the same export gives byte-identical files.
...
- every relationship traces to the FK cell, is `stated`, and any gap (column or table missing) is in `notes`
...
## Tests
`npm test` runs `node --test`. `test/fixtures/messy/` is a csv export with a title line above the header, merged-cell blanks, mixed-case identifiers, spaced types, a quoted comma in a definition, a PK disagreement, a duplicate column, a blank definition, an extra column, a keys-only table, a table with no key, an unreadable FK and a BOM. `test/helpers.js` builds `.xlsx` files (stored and deflated) so the workbook reader is tested against the same rows.
