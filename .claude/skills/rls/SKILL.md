---
name: rls
description: Add or edit a row-level-security policy, the roles-to-SSO-groups mapping, or an exemption in security/rls-policies.yaml. Step 5, and whenever access rules change.
...
- `/rls roles`: edit the roles to SSO groups mapping.
...
## Stops when
- `<view>` has no file in `bi_model/` → name it; `/build-model draft` comes first.
...
## After it
Nothing runs. The user continues step 5: `/build-model finalize` opens the PR with this file changed, and CODEOWNERS routes review to security and the data owner. Enforcement is `packages/semantic-query` (step 10); proof is `/verify` (step 11).
