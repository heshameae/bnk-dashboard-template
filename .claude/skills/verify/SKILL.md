---
name: verify
description: Step 11 — check every widget against finance's numbers in catalog/acceptance.yaml and prove RLS; writes dashboards/<name>/verify.md
disable-model-invocation: true
---

# /verify — the numbers finance would bet on
...
## Writes
- `dashboards/<name>/verify.md`, shaped like `dashboards/cashboard/verify.md` (example)
...
## Hands over to
PASS → merge; the dashboard is live in the portal's Dashboards tab, and `/ask-leap-bi <dashboard>` reports `live`. Anything else → the step named in `Go back to:`. A number disputed after go-live starts here too: PASS means the recipe matches finance and the dispute is about meaning (step 6); FAIL means the model (step 5).
