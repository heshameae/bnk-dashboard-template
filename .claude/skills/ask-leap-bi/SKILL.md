---
name: ask-leap-bi
description: The map of the platform's skills and where a dashboard is on it — the main flow, the on-ramps, where a session starts and ends, and the exact next command.
...
1. **`/import-schema`** files the data team's dictionary and profile, once per source system. Merge onto the flow at step 2 when the tables a dashboard needs are already filed.
...
3. **`/model draft`** writes the clean tables, DRAFT recipes, the proof pack and a policy stub. Nothing runs.
...
5. **`/model finalize`** judges the proofs, runs the checklists, opens the PR. **`/rls <view>`** for every new view, in the same session, before the PR.
...
- **Step 4 is a hard boundary.** A data engineer works for days. Start a fresh session for `/model finalize`; there is nothing to carry, because the proof results are files.
...
2. Walk the decision list top to bottom; the first rule that matches names the step. Completion: exactly one step chosen.
   - No `sources/*.yaml` covering a table the business-context §5 names → **step 1** (owner: data engineers; you file it). Next: `/import-schema <handover>`.
   - `business-context.md` absent → **step 2**. Next: `/business-context <name> <notes>`.
   - Every §3 row carries `reuse recipe <id>` and each id is CONFIRMED → the model exists; continue from the step 8 rule.
   - A §3 row without a `reuse recipe` mark and no recipe with `introduced_by: <name>` → **step 3**. Next: `/model draft <name>`.
   - A `bi_model/proofs/<view>.sql` exists with no matching `.md`, or the `.md` has a row that is not PASS → **step 4** (owner: data engineers). Next: send them the proof files; then `/model finalize <name>`.
   - All proofs PASS and no open PR touches `bi_model/` for these views → **step 5**. Next: `/model finalize <name>`; `/rls <view>` for any view without a policy entry.
...
3. Print the report in this shape. Completion: all five lines present.
   ...
   files:    business-context ✓ · views ✓ (proofs PASS) · recipes 2 CONFIRMED / 2 DRAFT · acceptance 2/2 · spec BLOCKED · queries — · verify —
...
## Hands over to
The owner of the detected step, with the command printed on the `next:` line.
