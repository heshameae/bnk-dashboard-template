---
name: spec
description: Bind the design export to recipes and write dashboards/<name>/spec.yaml as READY or BLOCKED. Step 9.
...
## After it
Nothing runs. When status is READY the user runs `/build <dashboard>`. Each BLOCKED item goes where the user answered: step 3 (`/build-model draft`, a new recipe) or step 8 (a design change). READY widgets can be previewed meanwhile.
